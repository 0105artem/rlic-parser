...
from db.models.card import DBCardFields
...
class ParserProcess:
    def __init__(self, redis_pool: redis.ConnectionPool, urls: List[str], worker_id: int,
                 redis_q: str = "db_cards_queue") -> None:
        self.tables_queue = asyncio.Queue()
        self.cards_queue = asyncio.Queue()
        self.redis_q: str = redis_q
        self.urls = urls
            ...
            while self.tables_queue.qsize():
                ...
                for res in results:
                    ...
                    if res['status_code'] == 200:
                        ...
                        for card_id in res['cards_ids']:
                            url = f'https://islod.obrnadzor.gov.ru/rlic/details/{card_id}/'
                ...
                if self.cards_queue.qsize():
                    ...
                    for res in results:
                        ...
                        if res['status_code'] == 200:
                            ...
                            self.redis_client.lpush(self.redis_q, str(card))
                        elif res['status_code'] >= 500:
                            logger.error(f"Failed to parse card {res['card_id']} from {res['url']}. "
                                         f"Response status code -- <{res['status_code']}>. "
                                         f"Is server available?")
                            available = await self.check_server(url='https://islod.obrnadzor.gov.ru/rlic/')
                            if available:
    ...
    async def get_table(client: Client, url: str, payload: dict = None) -> dict:
        ...
        :return: {url: table url, status_code: response status code, table_id: table's page number,
                  cards_ids: IDs of cards that managed to parse}
        """
        ...
        if status_code == 200:
            ...
            cards_ids = RlicParser.parse_table(html_doc)
        else:
            cards_ids = None

        return {'url': url, 'status_code': status_code, 'table_id': table_id, 'cards_ids': cards_ids}
    ...
    async def get_card(client: Client, url: str) -> dict:
        ...
        :param url: Card url
        :return: {url: card url, status_code: response status code, card_id: unique card's id,
                  card: parsed card}
        """
        card_id = url[:-1].split('/')[-1]
        ...
        return {'url': url, 'status_code': status_code, 'card_id': card_id, 'card': card}
